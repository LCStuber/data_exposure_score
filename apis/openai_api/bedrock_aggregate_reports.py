...
# --- your existing constants / lists / functions (unchanged) ---
ageRanges = [
...
def summarize_aggregation_with_qwen(agg: Dict[str, Any]):
    """
    Builds a short prompt and asks the Qwen3 model to summarize the aggregation.
    The prompt requests a concise bulleted summary + a JSON object with high-risk months, so you can parse it.
    """
    prompt = (
        "You are an assistant that summarizes aggregated analytics results. "
        "Produce a short, clear summary (3-6 bullets) in Portuguese and then a JSON object containing "
        "keys: 'summary' (string with bullets), 'high_risk_months' (list of month strings with count_gt_800>0), "
        "'top_age_ranges' (list of age ranges with highest avg_des)."
        "\n\nDados (JSON):\n"
        + json.dumps(agg, ensure_ascii=False)
    )
    try:
        result = invoke_bedrock_model(prompt, max_tokens=800)
        print("[BEDROCK] Resposta do modelo Qwen3:")
        print(result)
        ...
        if isinstance(result, str):
            ...
            if start != -1:
                try:
                    j = json.loads(result[start:])
                    print("[BEDROCK] Parsed JSON from model:")
                    print(json.dumps(j, ensure_ascii=False, indent=2))
                except Exception:
                    pass
    except Exception as e:
        print(f"[WARN] Falha ao chamar Bedrock/Qwen3: {e}")
...
def main():
    ...
    # ----------------------------
    # Optional: call Bedrock / Qwen3 to summarize the aggregation
    # ----------------------------
    # To enable: set env BEDROCK_MODEL_ID (ex: "qwen3-32b" or the exact modelId seen in Bedrock console)
    # Optionally set RUN_BEDROCK_SUMMARY=1 to force invocation
    try:
        run_bedrock = os.getenv("RUN_BEDROCK_SUMMARY", "0") == "1"
        bedrock_model_id = os.getenv("BEDROCK_MODEL_ID")
        if run_bedrock or bedrock_model_id:
            print("[INFO] Chamando Bedrock/Qwen3 para gerar resumo...")
            summarize_aggregation_with_qwen(agg)
        else:
            print("[INFO] BEDROCK_MODEL_ID não definido e RUN_BEDROCK_SUMMARY!=1 -> pulando chamada ao Bedrock.")
    except Exception as e_beds:
        print(f"[WARN] Erro ao tentar gerar resumo via Bedrock: {e_beds}")
